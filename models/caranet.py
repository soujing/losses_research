...
if __name__ == '__main__':
    images = torch.rand(1, 3, 224, 224).to(DEVICE) #.cuda(2)
    model = res2net50_v1b_26w_4s(pretrained=True)
    model = model.to(DEVICE) #.to(DEVICE) #.cuda(2)
...
if __name__ == '__main__':
    ras = caranet().to(DEVICE) #.cuda(2)
    input_tensor = torch.randn(1, 3, 352, 352).to(DEVICE) #.cuda(2)
